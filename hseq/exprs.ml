...
(* const_key: identifier of constants in a polynomial expressions
   e.g. in polynomial expressions `1+2*x'; term 2*x has key x and term
   1 has key const_key
*)
let const_key = 0
...
struct
  ...
  let rec expr_string t =
    match t with
      ...
      | PosInf -> "pinf"
      | NegInf -> "ninf"
      | Plus(xs) -> "Plus ("^(list_string expr_string ", " xs)^")"
      | (Mult(a, b)) -> 
  ...
  let print t = 
	...
	| Max tl ->
	  Format.printf "@[<2>max(";
	  Printer.print_sep_list (print_aux, ",") tl;
	  Format.printf ")@]@,"
	| Min tl -> 
	  Format.printf "@[<2>min(";
	  Printer.print_sep_list (print_aux, ",") tl;
    ...
    then b
	  ...
	  | Val(y) -> Val(v */ y)
          ...
          | _ -> 
      	    if b = PosInf 
            then 
              if v < zero_num then NegInf else b
      	    else 
	      if b = NegInf
              then if v < zero_num then PosInf else b
	      else Mult(v, b)
      end
...
let rec eval t =
  let reduce f y ys = 
    List.fold_left (fun a b -> Val(f (eval a) (eval b))) y ys
  in
  match t with
    | Val(x) -> x
    | Plus(y::ys) -> eval (reduce (+/) y ys)
    | Mult(x, y) -> x */ (eval y)
    | Max(y::ys) -> eval (reduce max y ys)
    | Min(y::ys) -> eval (reduce min y ys)
...
struct
  ...
  *)
  ...
  (** [set_term a c poly]: Set multiplicant of variable [a] to value
      [c] in polynomial [poly].  *)
  let set_term a c poly =  
	...
	| y::ys ->
	  let (n, v) = dest_term y 
          in 
	  if a = n 
	  then List.rev_append ys ((make_term n c)::ys)
	  else set_aux ys (y::rs)
    in 
    match poly with
      | Plus xs -> Plus(List.rev (set_aux xs []))
  ...
  *)
  ...
  let remove_empty ls =
    let pred t = not ((is_cnstr t)  && ((dest_cnstr t)=[]))
    in 
    List.filter pred ls

  (* [add c n poly]: If [poly = d + ax + by + ...], [add c n poly]
     calculates [(c * n) + poly].  *)
  let add v n poly =
    let rec add_aux xs  =
      match xs with 
  ...
  (** [mult c poly]: If [poly = d + ax + by + ...], [mult c poly]
      calculates [c * poly], where [c] is a constant.  *)
  let mult v poly =
    let rec mult_aux xs  =
      match xs with 
	| [] -> []
  ...
  (** Conversions between a simple expression and its polynomial
      form.  *)

  (** [poly_of_expr t]: Convert expression [t] to its polynomial
      form.  *)
  let poly_of_expr t =
  ...
  (* [unbundle prd dst xs]: Apply destructor [dst] to every [x] in
     [xs] which satisfies [prd] to obtain a list [y]. Return the list
     of elements of [xs which don't satisfy [prd] concatenated to the
     lists resulting from the destructor.
  ...
  *)
  ...
  let distrib trm = 
	...
	| Max([]) -> adds
	| Max(x::xs) -> Max(List.map (top m adds) xs)
	| Min(xs) -> Min(List.map (top m adds) xs)
	| Plus([]) -> adds
	| Plus(x::xs) ->
	  let i = top m (Plus[]) x
	  in 
...
struct
  ...
  *)
  let remove_var_rhs x (a, b) = 
    ...
    match bv with
      | Some(bc) -> 
	(Poly.add (Val(minus_num bc)) x a, 
         Poly.add (Val(minus_num bc)) x b)
      | _ -> (a, b)
  ...
  *)
  ...
  (** [find_soln x (a, b)]: Find a value for variable [x] which
      makes [a=<b] true.  *)
  let find_soln x (a, b) = 
    (* remove x from b, adding to a *)
    let na, nb = remove_var_rhs x (a, b) in 
      (* remove c+ax+.. from na, adding to nb *)
    let na1, nb1 = do_subs x (na, nb) in 
  ...
  (** [solve ts x]: try to solve list of inequalities [ts] for
      [x].  *)
  let solve ts x= 
  ...
  (** [solve_for x ts]: Solve list of inequalities [ts] for variable
      [x].

      The list of inequalities is of the form [(a, b); ...; ] where
      [(a, b)] represents [(a=<b)] and [a] and [b] are exprs.

      If [x] has a solution, results in lists [(L1, x); (L2, x); ...],
      [(x, U1); (x, U2), ... ] [(a, b);...]  where variable [x] does
      not occur in the [a], [b], [L] or [U].  *)
  let solve_for x ts=
    let from_exprs rs = 
      List.map 
    ...
    in 
    let lt, gt, oths = solve (from_exprs ts) x
    in 
