...
(** [reduce_conv scp t]: Conversion to simplify term [t].  Term [t]
    must be in the set which can be dealt which by SupInf.  Returns a
    theorem of the form [ |- t=t' ] where [t'] is the simplified term.

    [decide_conv scp t]: Conversion to decide whether [t] is true.
    Term [t] must be in the set which can be dealt which by SupInf.

    Returns a theorem of the form [ |- t=t' ] where [t'] is either
    true or false.
*)
...
(** [is_numterm scp t]: [true] if term [t] is a number, an integer
    ...
    Used to see whether term [t] is suitable for simplifying (with
    Exprs.simp).
*)
...
(** [is_presburger scp t]: [true] if term [t] is a possibly
    universally quantified boolean term in which subterms are
    quantifier-free boolean terms (satisfying [is_boolterm])

    Used to see whether term [t] is suitable for the decision
    procedure Supinf.decide.
*)
