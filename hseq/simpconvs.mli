...
(** [simple_rewrite_conv scp rule trm]
    ...
    applying rewrite once only to the body of [trm]. Return the
    theorem stating the equality [(!x..y: body)=(!x.. y: (body=r))].
...
val simple_rewrite_rule:
  Context.t -> Logic.thm -> Logic.thm -> Logic.thm
...
(** [simple_asm_rewrite_tac rule l asm]
    ...
    universal quantifiers and applying rewrite once only to the body
    of [asm].  i.e.
...
(** [is_many_conj thm]: true if [thm] is of the form [ |- a and b and
    ... and z ].
*)

val is_neg_disj: Logic.thm -> bool
(** [is_neg_disj thm]: true if [thm] is of the form [ |- not (a or b)
    ].
*)
...
val is_constant:
  Term.term list
  -> Term.Binder.t list * Term.term option * Term.term -> bool
...
val is_neg_all:  Term.Binder.t list * Term.term option * Term.term -> bool
(**  [is_neg_all (var, cnd, trm)]: [trm] is [not ! a: b] *)

val is_neg_exists: Term.Binder.t list * Term.term option * Term.term -> bool
(** [is_neg_exists (var, cnd, trm)t]: [trm] is in the form [not ? a:
    b] *)

val is_rr_rule:
  (Term.Binder.t list * Term.term option
...
val is_rr_equality: Term.Binder.t list * Term.term option * Term.term -> bool
...
sig
  ...
  val accept_all_thms:
    Logic.thm list ->
    (Context.t * Logic.thm
     * (Term.Binder.t list * Term.term option * Term.term))
    -> Logic.thm list
  ...
  *)
  ...
  (** Convert [|- not a] to [|- a=false] and [|- c=> not a] to [|- c=>
      ...
      c] and [not (a or b)] -> [(not a) and (not b)].  *)
...
val thm_to_rules:
  Context.t -> Logic.thm -> Logic.thm list
...
(** Functions to convert an assumption
    ...
    [eq_asm]: Convert [a=b] to [(a=b) = true] and [(b=a)=true] and
    [c=> (a=b)] to [c=>((a=b) = true)] and [c=>((b=a)=true)] This is
    only called if [rr_equality_asm] failed.

    [fact_rule_asm]: Convert [a] to [a=true] and [c=> false] to [(not
    c)] and [c=> a] to [c => a=true] pass [(a=b)] and [c=>(a=b)] to
    [eq_asm] and solve [false |- C]

    [neg_eq_asm]: Convert [not (a=b)] to [(a=b) = false] and
    [(b=a)=false] and [c=>not (a=b)] to [c=>((a=b) = false)] and
    [c=>((b=a)=false)]

    [neg_rule_asm]: Convert [c=> not true] to [not c] and [not (a or
    b)] to [(not a) & (not b)] and [not a] to [a=false] and [c=> not
    a] to [c=> a=false]; [ (c => not a) = true] pass [not (a=b)] and
    [c=>not(a=b)] to [neg_eq_asm] and solve [not true |- C]
...
sig
  ...
  *)
  ...
  (** Convert and [c=> not true] to [not c] and [not a] to [a=false]
      ...
      (a=b)] and [c=>not(a=b)] to [neg_eq_asm] and solve [not true |-
      C] *)
