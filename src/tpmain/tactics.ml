...
let repeat tac g = 
  Rules.repeat 
   (fun x -> (Rules.apply_flatten (Rules.sqnt_apply tac) x)) g
*)
(*
    let rec repeat tac g  =
      let ng = sqnt_apply tac g
      in 
      try
	(repeat tac) ng
      with _ -> ng
...
let thenl ts g = 
  Logic.Rules.thenl 
  (List.map (fun x -> (Rules.apply_flatten (Rules.sqnt_apply x)))ts) g
*)
    let (++) tac1 tac2 g =
      Logic.Rules.thenl [tac1; tac2] g

let apply_list ts g = Logic.Rules.apply_list ts g

(*
let apply_list ts g = 
  Logic.Rules.apply_flatten
  (Logic.Rules.apply_list 
   (List.map 
    (fun x -> (Rules.apply_flatten (Rules.sqnt_apply x)))ts)) g

let foreach t g = 
  Rules.apply_flatten
    (Logic.Rules.foreach (Rules.apply_flatten (Rules.sqnt_apply t))) g
*)
