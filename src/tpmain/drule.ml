...
(*
      match (Logic.implE i sq) with
	[] -> []
      |	[x; y] ->
	  (let c=get_cncl 1 x
	  in 
	  let rslt = Logic.unify 
	      (first_asm (Formula.alpha_convp (Tpenv.typenv()) c) x) 1 x
	  in 
	  match rslt with 
	    [] -> [y]
	  | _ -> Result.raiseError "mp rule")
      |	 _ -> Result.raiseError "mp rule"
*)
