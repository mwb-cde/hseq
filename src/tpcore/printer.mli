...
(** 
   A printer [print: 'a printer] prints the representation of 
   an object [print obj].
*)
...
(** 
   Two sets of functions to manipulate printer [info]. [*_record] deal
   with printer records, [*_info] construct the record to be
   added/accessed.
*)
...
(** 
   [print_space info]
   Print a space (using Format.print_space)
*)
...
(**
   ...
   then just print the identifier.
*)

val print_infix: 
    ((int -> ('a) printer) * (int -> ('b list) printer))
    -> int -> ('a  * ('b) list) printer
(**
   [print_infix (opr, tpr, spr) prec (op, trm)]: Infix Printer.  Print
   first element [trm] using printer [tpr], print [op] using [opr]
   print rest of [trm] using [print_list (tpr prec, spr)]. If [trm] is
   empty, just print [op].
 *)
...
val print_suffix: 
  ...
  -> int ->  ('a * ('b) list) printer
(**
   [print_suffix (tpr, spr) prec (op, trm)]: Suffix printer.
   Print elements [trm] using [print_list (tpr prec, spr)], 
   print [op] using [opr].
   If [trm] is empty, just print [op].
 *)
