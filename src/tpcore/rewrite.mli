...
(**
   Rewrite rules are terms of the form [x=y]. Rules can be ordered by
   a predicate [order]. Ordered rewrite rules are only applied to a
   term [t] if [order x t] is true, where [order x t] is interpreted
   as [x] is less-than [t].
 *)
...
(** 
   [plan_rewrite scp ?dir tyenv p t]: Rewrite term [t] with plan
   [p].
 *)
...
module Planner :
  sig
	...
	val dest : (** How to destruct rewrite rules. *)
	    data 
	  -> rule 
	    -> (Basic.binders list * Basic.term * Basic.term * order option)
      end
...
(** Rewrite plan constructors.
   ...
   {!Rewrite.Planner.T.make}, is for general rewriting. The second,
   {!Rewrite.Planner.T.make_env}, for rewriting w.r.t a type
   context.
...
(** 
   [mk_branches ps]: Rewrite the subnodes of the node with plans [ps],
   applying the i'th plan to the i'th subnode.
 *)
...
module TermPlannerData :  
    (Planner.Data with type rule = Basic.term 
    and type data = unit)
(** Data used to instantiate the generic planner. *)

module TermPlanner : 
    (Planner.T with type a_rule = TermPlannerData.rule 
