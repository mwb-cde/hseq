...
(** Theories.
    ...
    then all of its parents. A theory to which additions can be made
    is {e writable} and none of the parents of a theory can be
    writable. A {e protected} theory is not writable. A theory also
    stores a, possibly empty, list of files to be loaded when the
    theory is first used.
...
(** [type property]: Properties of theorems. Properties are string to
    allow arbitrary new properties to be declared.
*)
...
val get_term_pplist: 
  thy -> (Hident.t * (Printer.record * sym_pos)) list
...
val get_type_pplist: thy -> (Hident.t * Printer.record) list
...
(** {5 Theory Storage} 
    ...
    theory scope, the parents of the theory must be in scope. The
    first stage of loading a theory is to use function
    {!Theory.load_theory} to load the theory as a [saved_thy]. This
    can be interogated to get the information needed to load the
    theory parents and then to form a scope [scp] for the theory. In
    the second stage, function {!Theory.from_saved} is used to convert
    the theory to the [thy] representation, using [scp] to make a
    scope in which to rebuild the theory formulas.

    Functions in {!Thydb.Loader} normally handle loading a theory.
*)
...
(** Convert a theory from the permanent storage representation read
    from an channel.
*)
...
(** Convert a theory to permanent storage representation and emit to
    an output channel.
*)
