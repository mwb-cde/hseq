...
(** The record holding information generated by tactics. *)
  ...
  let empty_changes () = 
    { goals = []; aforms = []; cforms = []; terms = [] }
  let make_changes gs hs cs ts = 
    { goals = gs; aforms = hs; cforms = cs; terms = ts }
  let add_changes r gs hs cs ts =
    make_changes (gs@r.goals) (hs@r.aforms) (cs@r.cforms) (ts@r.terms)

  let sum_changes l r =
    make_changes 
      (List.rev_append l.goals r.goals) 
      (List.rev_append l.aforms r.aforms)
      (List.rev_append l.cforms r.cforms)
      (List.rev_append l.terms r.terms)
  ...
  let combine_changes l r =
    sum_changes (rev_changes l) r
...
struct
  ...
  let new_sqnt info scp x = 
    let env = mk_sqnt_env [] scp [] []
    and gtag = Tag.create()
    and cform = mk_sqnt_form x
    in 
    do_info info [gtag] [] [form_tag cform] [];
    make gtag env [] [cform]
...
*)
type goal = 
    Goal of (Sequent.t list * Gtypes.substitution * Formula.t * New.changes)

let get_goal (Goal(_, _, f, _)) = f
let get_subgoals (Goal(sq, _, _, _)) = sq
let goal_tyenv (Goal(_, e, _, _)) = e
let goal_chngs (Goal(_, _, _, c)) = c
...
let mk_goal ?info scp f = 
  let goal_frm = Formula.typecheck scp f (Lterm.mk_bool_ty())
  in 
  Goal([Sequent.new_sqnt info scp goal_frm], 
       Gtypes.empty_subst(), 
       goal_frm, 
       New.empty_changes())
...
struct
  ...
  (** Subgoals: *)
  type node = Node of (Tag.t * Gtypes.substitution * Sequent.t * New.changes)
  ...
  type branch = 
      Branch of (Tag.t * Gtypes.substitution * Sequent.t list * New.changes)
  ...
  *)
  ...
  let merge_tac_tyenvs n b =
    ...
    in 
    let nbchngs = New.combine_changes (node_changes n) bchngs
    in
  ...
  *)
  let apply_to_each tac (Branch(tg, tyenv, sqnts, chngs)) =
    let rec app_aux ty gs cs lst =
      match gs with
	| [] -> mk_branch tg ty (List.rev lst) (New.rev_changes cs)
	| (x::xs) ->
	  let branch1 = 
	  ...
	  app_aux 
            ...
            (New.sum_changes (branch_changes branch1) cs)
	    (List.rev_append (branch_sqnts branch1) lst)
    in 
    match sqnts with
      | [] -> raise No_subgoals
      | _ -> app_aux tyenv sqnts chngs []
  ...
  *)
  let rule_apply r (Node(ntag, tyenv, sqnt, chngs)) =
    try
     ...
     with 
       | No_subgoals -> 
         Branch(ntag, tyenv, [], New.empty_changes())
       | Solved_subgoal ntyenv -> 
         Branch(ntag, ntyenv, [], New.empty_changes())
...
struct
  ...
  *)
  let copy_asm0 ?info l sq = 
    ...
    in 
    let chngs = New.make_changes [] [nt] [] [] in
  ...
  let cut0 ?info x sq=
    ...
    try 
      let ng = mk_subgoal(Sequent.sqnt_retag sq, 
		          ...
		          Sequent.concls sq)
      in 
      let chngs = New.make_changes [] [ftag] [] []
      in
      (ng, chngs)
...
***)
  ...
  *)
  let conjA0 ?info i sq =
    ...
    then 
      ...
      and asm2 = (ft2, t2)
      in 
      let chngs = New.make_changes [] [ft1; ft2] [] [] in
      let sbgl = 
        mk_subgoal (Sequent.sqnt_retag sq, Sequent.sqnt_env sq, 
  ...
  *)
  let conjC0 ?info i sq=
    ...
    then 
      let (t1, t2) = Formula.dest_conj t
       ...
       in 
       let chngs = New.make_changes [tagl; tagr] [] [ft1] [] in
       let sbgl = 
         [
  ...
  let disjA0 ?info i sq=
    ...
    then 
      ...
      and tagr = Tag.create()
      in 
      let chngs = New.make_changes [tagl; tagr] [ft] [] [] in
      let sbgl = 
        [ 
  ...
  *)
  let negA0 ?info i sq =
    ...
    then 
      ...
      let cncl1 = (ft, t1)
      in 
      let chngs = New.make_changes [] [] [ft] [] in
      let sbgl = 
        mk_subgoal (Sequent.sqnt_retag sq, 
		    Sequent.sqnt_env sq, 
		    join_up lasms rasms,
  ...
  *)
  let implA0 ?info i sq =
    ...
    then 
      ...
      and tagr = Tag.create()
      in 
      let chngs = New.make_changes [tagl; tagr] [ft] [ft] [] in
      let sbgl =
        [
  ...
  *)
  let implC0 ?info i sq =
    ...
    then 
      ...
      and cncl = (ft1, t2)
      in 
      let chngs = New.make_changes [] [ft2] [ft1] [] in
      let sbgl = 
        mk_subgoal
	  (Sequent.sqnt_retag sq, 
	   Sequent.sqnt_env sq, 
  ...
  let allC0 ?info i tyenv sq =
	...
	then sty::(Sequent.sqnt_tyvars sq)
      ...
      in 
       ...
       in 
       let chngs = New.make_changes [] [] [ft] [sv]
       in
       (new_subgoal, gtyenv, chngs)
    else raise (logic_error "Not a universal quantifier" [t])
  ...
  *)
  ...
  (** [rewrite_intro ?info plan trm sq]: Introduce an equality
  ...
  *)
  let rewrite_intro0 ?info plan trm tyenv sq =
	...
	Gtypes.extract_bindings (Sequent.sqnt_tyvars sq) ntyenv tyenv 
	  ...
	  (Sequent.sqnt_retag sq, 
	   ...
	   Sequent.concls sq)
      in 
      let chngs = New.make_changes [] [asm_tag] [] []
      in
      (new_subgoal, gtyenv, chngs)
    in 
    try do_rewrite()
  ...
  (** [substA ?info eqs l sq]: Substitute, using the assumptions in
  ...
  *)
  let substA0 ?info eqs l tyenv sq =
	...
	Gtypes.extract_bindings (Sequent.sqnt_tyvars sq) tyenv2 tyenv
	  ...
	  (Sequent.sqnt_retag sq, 
	   ...
	   Sequent.concls sq)
      in 
      let chngs = New.make_changes [] [form_tag] [] []
      in
      (new_subgoal, gtyenv, chngs)
    in 
    try do_subst()
  ...
  (** [substC ?info eqs l sq]: Substitute, using the assumptions in
  ...
  *)
  ...
  (** [nameA ?info name l sq]: Rename the assumption labelled [l] as
  ...
  *)
  ...
  let nameA0 ?info name lbl sqnt = 
    let name_aux () = 
      ...
      let chngs = New.make_changes [] [new_tag] [] [] in
      let new_sbgl = 
        mk_subgoal 
...
struct
  ...
  *)
  let prove_subtype_exists scp setp thm =
    ...
    let info = ref empty_tag_record in 
    let gl = mk_goal ~info:info scp goal_form in 
    let concl = FTag(List.hd (!info).cforms) in
    let tac1 g = Tactics.cut ~info:info thm g in 
    let tac2 g =
