...
(* Substitution primitives *)
    ...
    let subst_size = Term.subst_size 
    let empty_subst () = subst_size 5
...
(* closed terms and conversion to formula *)
	...
	Term.App(l, r) -> is_closed_scope env l; is_closed_scope env r
      ...
      |	Term.Qnt(q, b) -> 
	  ignore(Term.add (Term.Bound(q)) (Term.mkbool true) env);
	  is_closed_scope env b;
	  Term.remove (Term.Bound(q)) env
      |	Term.Bound(q) -> 
	  (try ignore(Term.find t env)
	  with Not_found -> 
	    raise (Term.termError  "Not closed"  [t]))
      |	_ -> ()

    let is_closed t = 
      try is_closed_scope (Term.empty_subst()) t; true
      with _ -> false
...
let is_form scp t =
  ...
  let rec in_scope x=
	...
	ignore(lookup_id id); ignore(lookup_ty ty)
	  ...
	  ignore(Term.add (Term.Bound(q)) (Term.mkbool true) qnt_env);
	  in_scope b;
	  Term.remove (Term.Bound(q)) qnt_env
      |	Term.Bound(q) -> 
	  ignore(Term.find t qnt_env)
...
let form_of_term scp t0 = 
  ...
  in Typing.retype_pretty tenv t
...
(* substitution: term substitution followed by check for closed term *)
    ...
    let subst scp env t = 
      let nt = Term.subst_env env (term_of_form t)
      in 
      form_of_term scp nt
...
let rename t = Term.rename t
	...
	in 
  	let f= (Term.subst (Term.Bound(q)) nr b)
	in 
	  f)
