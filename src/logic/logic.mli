...
(** Theorems and rules of the logic.
    ...
    name is independent of the position of the formula and is
    therefore more robust than positional labels. The assignment of a
    name is handled by tactics {!Logic.Tactics.nameA} or
    {!Logic.Tactics.nameC}.
    ...
    consulted for specific details. Note that, because every subgoal
    is uniquely tagged, the standard tactics only record the subgoal
    tags if more than one subgoal is produced. The terms recorded are
...
module New: 
sig
(** {7 New approach to recording goal changes.} 
...
*)
  ...
  (** Construct a [tag_record]. *)
  val make_changes:
    Tag.t list -> Tag.t list -> Tag.t list -> Basic.term list -> changes
...
module Skolem:
sig
  (** Skolem constants are introduced by quantifier rules, to
...
(** The subgoal package. Manages the application of rules to the
    ...
    and a type {!Logic.Subgoals.branch} of branches, representing
    nodes and branches of the proof tree. A tactic has type [node ->
    branch], a node contains the subgoal which is the argument to the
    tactic and a branch has the list of subgoals resulting from the
    tactic. Each node and branch has a unique tag. When a tactic [tac]
    is applied to a node [n], a unique tag (the ticket) is generated
    and set as the tag of [n]. If branch [b=tac n], the tag of [b]
    must be the same as the ticket otherwise the result is invalid and
    will be discarded. If the result is valid, the tag of [b] is set
    to the original tag of [n] and [b] is the result of the tactic.
