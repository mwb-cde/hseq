...
let dest_rule (bs, cnd, lhs, rhs, rr)=(bs, cnd, lhs, rhs, rr)
...
let add_rule s rl=
  let _, (vs, _, c, b, r)=rl
  in 
  {basic=Net.insert lt (is_variable vs) rl s.basic;
   next=s.next}
...
*)
  ...
  let negate_concl info t g=
    ...
    in 
    let inf1, g1=
      apply_tag (fun x -> simp_case_tac x newtrm) g
    in 
    let ogt, ngt=
...
*)
	...
	Some(false), _ ->   (* c=>a=b --> (c=>a=b)=true) *)
	  apply_on_cond cnd
	    (thm_rewrite scp (get_rule_true_ax()))
	    (thm_cond_rewrite scp (get_rule_true_ax()))
	    thm
...
(* make_aux: main loop, test for different cases *)
	...
	then 
	  make_aux
	    (apply_on_cond cnd
	       (thm_rewrite scp (get_iff_equals_ax()))
	       (thm_cond_rewrite scp (get_iff_equals_ax()))
	       thm)
	    bl
	else 
	  ...
	  then 
	    make_aux
	      (apply_on_cond cnd
		 (thm_rewrite scp (get_rule_false_ax()))
		 (thm_cond_rewrite scp (get_rule_false_ax())) 
		 thm) bl
	  else 
   ...
   (* default *)
		...
		Some(false), _ -> 
		  make_aux (thm_rewrite scp (get_rule_true_ax()) thm) bl
	      |	_, _ ->
		  make_aux 
		    (apply_on_cond cnd
		       (thm_rewrite scp (get_rule_true_ax()))
 ...
 *)
  ...
  let rec make_asm_entry asmlist buildlist gl=
	...
	Some(false), _ -> 
	  let nasm, ng =
	    rewrite_cnd_tag cnd (get_rule_true_ax()) asm g
	  in (nasm, ng)
      |	_, Some(false) ->
	  let nasm, ng =
	    rewrite_cnd_tag cnd (get_rule_true_ax()) asm g
	...
	Formula.dest_form 
      ...
      then 
	let nasm, ng=
	  rewrite_cnd_tag cnd (get_iff_equals_ax()) asm g
	in make_aux nasm ng
      else 
	if is_negation (vars, cnd, main)
	then
	  let nasm, ng=
	    rewrite_cnd_tag cnd (get_rule_false_ax()) asm g
	  in make_aux nasm ng
	else 
	  if is_equality (vars, cnd, main)
	  then 
...
(* default *)
		...
		let nasm, ng=
		  apply_get_tag 
		    (fun inf -> asm_rewrite inf (get_rule_true_ax()) asm) g
		in make_aux nasm ng
	    | _, _ ->
		let nasm, ng=
		  rewrite_cnd_tag cnd (get_rule_true_ax()) asm g
...
let add_simp_rule sset scp src =
  let nset=ref sset
  in 
  let rec add_aux thms =
    match thms with
      [] -> !nset
    | rl::ts -> 
	nset:=add_rule (!nset) rl;
	add_aux ts
  in 
  add_aux (make_thm_entry scp src [])
...
let add_sqnt_asms sset asms sqnt =
  ...
  let rec get_asms nasms bl= 
    match nasms with 
      [] -> List.rev bl
    | ((_, (_, _, _, _, Logic.Tagged(t))) ::na) -> get_asms nasms (t::bl)
