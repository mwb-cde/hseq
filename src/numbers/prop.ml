...
let rec dnf_top n t =
  ...
  | Or(a, b) -> 
      if n
      then 
	dnf_top false (mk_and (mk_not a) (mk_not b))
      else
...
let mk_dnf x =
  disj_to_dnf(dnf_top false (Bool true) x) 
*)
...
let rec cnf_top n t =
  ...
  | And(a, b) -> 
      if n
      then
	cnf_top false (mk_or (mk_not a) (mk_not b))
      else 
       	mk_and (cnf_top n a) (cnf_top n b)
  | Or(a, b) -> 
      if n
      then 
	cnf_top false (mk_and (mk_not a) (mk_not b))
      else
	let na = (cnf_top n a)
	and nb = (cnf_top n  b)
	in push_disj na nb
...
let rec cnf_top n p t =
  ...
  | And(a, b) -> 
	...
	cnf_top false p (mk_or (mk_not a) (mk_not b))
       	...
       	mk_and (cnf_top n p a) (cnf_top n p b)
  | Or(a, b) -> 
      if n
      then 
	cnf_top false p (mk_and (mk_not a) (mk_not b))
      else
	let na = (cnf_top n p a)
	and nb = (cnf_top n (Bool false) b)
	in push_disj na nb
...
let mk_cnf x =
  conj_to_cnf(cnf_top false (Bool true) x) 
*)
...
(**
   [reduce p]:
   reduce proposition [p] as far as possible.
*)
let get_value p = 
  match p with
    Bool(b) -> Some(b)
...
let rec reduce p = 
  ...
  | And(l, r) -> 
      ...
      (match (lv, rv) with
	None, None -> And (l1, r1)
      | None, Some(x) -> if x then l1 else Bool(false)
      | Some(x), None -> if x then r1 else Bool(false)
      | Some(x), Some(y) -> if (x && y) then Bool(true) else Bool(false))
  | Or(l, r) -> 
      let l1= reduce l
      and r1 = reduce r
      in 
      ...
      (match (lv, rv) with
	None, None -> Or(l1, r1)
      | None, Some(x) -> if x then Bool(true) else l1
      | Some(x), None -> if x then Bool(true) else r1
      | Some(x), Some(y) -> if (x || y) then Bool(true) else Bool(false))
  | Implies(l, r) -> 
      reduce (mk_implies l r)
  | Iff(l, r) -> 
      reduce (mk_iff l r)
