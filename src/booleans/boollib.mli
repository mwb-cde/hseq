...
(**
s   [bool_eq_thm]: |- !x y: x iff y = ((x => y) and (y=>x))
 *)
...
(** 
   [once_rewrite_tac info dir f thms]: Rewrite formula [f] once.
...
val rewriteC_tac: 
  ...
  -> ?dir:Rewrite.direction
	...
	-> Logic.tactic
(** 
   [rewriteC_tac info dir c thms]: Rewrite conclusion [c] with list of
...
val rewriteA_tac: 
  ...
  -> ?dir:Rewrite.direction
	...
	-> Logic.tactic
(** 
   [rewriteA_tac info dir a thms]: Rewrite assumption [a] with list of
...
(**
   ...
   See {!Boollib.induct_tac} for details about the form
