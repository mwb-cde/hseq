...
let add_term_pp s ?(pos=Lib.First) i f sym = 
  Commands.add_term_pp 
    (Hident.mk_long (Global.current_name()) s) ~pos:pos i f sym
let get_term_pp s = 
  Commands.get_term_pp (Hident.mk_long (Global.current_name()) s)
let remove_term_pp s = 
  Commands.remove_term_pp (Hident.mk_long (Global.current_name()) s)

let add_type_pp s = 
  Commands.add_type_pp (Hident.mk_long (Global.current_name()) s)
let get_type_pp s = 
  Commands.get_type_pp (Hident.mk_long (Global.current_name()) s)
let remove_type_pp s =
  Commands.remove_type_pp (Hident.mk_long (Global.current_name()) s)
